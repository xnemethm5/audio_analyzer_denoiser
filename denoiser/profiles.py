...
Parametre `strength_*` sa používajú ako EXPONENT na MMSE-LSA gain:
  ...
  strength < 1.0  → mäkšie filtrovanie
  strength > 1.0  → agresívnejšie filtrovanie
"""
...
class DenoiseProfile:
    # Sily filtrovania per frekvenčné pásmo (EXPONENT na MMSE-LSA gain)
    ...
    alpha_ns:   float = 0.96  # MMSE-LSA DD vyhladzovanie (0.92=rýchle, 0.98=hladké)
    window_sec: float = 0.6   # minimum statistics okno v sekundách
    bias:       float = 1.6   # bias korekcia minimum statistics
    n_fft_ms:   float = 32.0  # FFT okno v ms
    mask_floor: float = 0.05  # minimálna hodnota masky (0.05 ≈ −26 dB floor)


# Frekvenčné pásma v spectral.py:
...
#
#                                     s_lo s_mid s_hi  alpha  win  bias fft_ms floor
GENRE_PROFILES: dict[str, DenoiseProfile] = {
    # Classical: jemné tóny, dlhý sustain, pomalé zmeny
    "classical":  DenoiseProfile(0.80, 0.78, 0.72, 0.98, 1.0, 1.5, 46.0, 0.04),
    # Jazz: činely definujúce, jemné brush na bicích
    "jazz":       DenoiseProfile(0.82, 0.78, 0.72, 0.98, 1.0, 1.5, 46.0, 0.04),
    # Blues: gitarové slides, harmonika, sustain
    "blues":      DenoiseProfile(0.85, 0.82, 0.78, 0.97, 0.8, 1.6, 40.0, 0.04),
    # Pop: hlas dominantný v 200–3 kHz
    "pop":        DenoiseProfile(0.92, 0.90, 0.85, 0.96, 0.6, 1.6, 32.0, 0.05),
    # Country: akustická gitara, husle, banjo
    "country":    DenoiseProfile(0.88, 0.86, 0.82, 0.97, 0.8, 1.6, 40.0, 0.04),
    # Reggae: silný bas (40–120 Hz), skiffle drums
    "reggae":     DenoiseProfile(0.90, 0.90, 0.85, 0.96, 0.6, 1.6, 32.0, 0.05),
    # Rock: elektrická gitara, bicie – rýchlejší MMSE
    "rock":       DenoiseProfile(0.98, 0.95, 0.90, 0.94, 0.5, 1.8, 25.0, 0.06),
    # Hip-hop: sub-bas a 808, transienty dôležité
    "hiphop":     DenoiseProfile(0.95, 0.98, 0.95, 0.94, 0.5, 1.8, 25.0, 0.06),
    # Disco: činely, sláčiková sekcia v strede
    "disco":      DenoiseProfile(0.95, 0.92, 0.88, 0.94, 0.5, 1.8, 25.0, 0.06),
    # Metal: husté spektrum, rýchle transienty
    "metal":      DenoiseProfile(1.05, 1.00, 0.95, 0.92, 0.4, 2.0, 20.0, 0.07),
    # Electronic: úmyselný šum v padoch – konzervatívnejšie
    "electronic": DenoiseProfile(0.85, 0.85, 0.80, 0.94, 0.5, 1.6, 25.0, 0.05),
    # Default: vyvážený základ pre neznámy žáner
    "default":    DenoiseProfile(0.92, 0.90, 0.85, 0.96, 0.6, 1.7, 32.0, 0.05),
}
...
def get_profile(genres: list[dict] | None) -> tuple[DenoiseProfile, str]:
    ...
    if top["probability"] >= 60.0:
        ...
        return profile, f"{top['genre']} ({top['probability']}%)"
    ...
    return mixed, f"{g1['genre']} + {g2['genre']} (mix {w1:.0%}/{w2:.0%})"
...
def adapt_profile(
    ...
    Impulzný    (skóre > 0.3)  → vyšší floor, kratší window
    ...
    # --- Impulzný šum ---
    # Declicker rieši impulzy pred MMSE, tu len zvýšime floor
    if impulsive > 0.3:
